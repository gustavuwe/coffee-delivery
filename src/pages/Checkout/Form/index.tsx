...
const checkoutFormSchema = zod.object({
  ...
  street: zod.string().min(3).max(30),
  number: zod.string().min(1).max(13),
  complement: zod.string().min(1).max(30),
  neighborhood: zod.string().min(1).max(30),
  city: zod.string().min(1).max(30),
...
export function CheckoutForm() {
  const { coffees, totalCoffeesSelected, SetOrderData } = useContext(CoffeesContext);
  ...
  return (
    <>
      {totalCoffeesSelected > 0 ? (
        <CheckoutFormContainer>
          <LeftForm onSubmit={handleSubmit(handleSubmitOrder)}>
            <div>
            ...
            <PaymentContainer>
              <PaymentForm>
                ...
                <ButtonsContainer>
                  <Button
                    isSelected={selectedButton === "credit-card"}
                    onClick={(e) => {
                      e.preventDefault();
                  ...
                  <Button
                    isSelected={selectedButton === "debit-card"}
                    onClick={(e) => {
                      e.preventDefault();
